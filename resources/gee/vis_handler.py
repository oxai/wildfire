...
def array_to_image(image):
    if image.shape[0] == 3: image = np.concatenate([image,np.ones((1,256,256))],axis=0)
    image = np.maximum(0,np.minimum(255,(image * 255).astype('uint8').transpose(1, 2, 0)))
...
def vis_enhanced_natural_colors(ee_product, image, vis_params): 
    ...
    B3 = get_band(ee_product,image,'B3')
    B4 = get_band(ee_product,image,'B4')
    return [stretch((3.1 * B4 + 0.1 * B5),0.05,0.9), stretch((3 * B3 + 0.15 * B8),0.05,0.9), stretch(3 * B2,0.05,0.9)];
...
def vis_natural_nirswirmix(ee_product, image, vis_params): 
    ...
    B8 = get_band(ee_product,image,'B8')
    B12 = get_band(ee_product,image,'B12')
    return [stretch((2.1 * B4 + 0.5 * B12),0.01,0.99), stretch((2.2 * B3 + 0.5 * B8),0.01,0.99), stretch(3.2 * B2,0.01,0.99)]
...
def vis_fire10vl(ee_product, image, vis_params): 
    ...
    B8 = get_band(ee_product,image,'B8')
    B12 = get_band(ee_product,image,'B12')
    return [stretch((2.1 * B4 + 0.5 * B12),0.01,0.99)+1.1, stretch((2.2 * B3 + 0.5 * B8),0.01,0.99), stretch(2.1 * B2,0.01,0.99)]
...
def vis_fire20vl(ee_product, image, vis_params): 
    ...
    B8 = get_band(ee_product,image,'B8')
    B12 = get_band(ee_product,image,'B12')
    return [stretch((2.1 * B4 + 0.5 * B12),0.01,0.99)+1.1, stretch((2.2 * B3 + 0.5 * B8),0.01,0.99)+0.5, stretch(2.1 * B2,0.01,0.99)]
